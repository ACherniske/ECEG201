...
ahtMinIndex = np.argmin(ahtTemp)

mcpMaxIndex = np.argmax(mcpTemp)
mcpMinIndex = np.argmin(mcpTemp)
...
aht_Norm = normalize(ahtTemp, ahtTemp[ahtMinIndex], ahtTemp[ahtMaxIndex])
aht_offset_Norm = normalize(offset, ahtTemp[ahtMinIndex], ahtTemp[ahtMaxIndex])
aht_time_Norm = normalize(time[ahtMaxIndex:299], time[ahtMaxIndex], time[299])
aht_Temptau = unnormalize(aht_offset_Norm + 1/np.e, ahtTemp[ahtMinIndex], ahtTemp[ahtMaxIndex])

mcp_Norm = normalize(mcpTemp, mcpTemp[mcpMinIndex], mcpTemp[mcpMaxIndex])
mcp_offset_Norm = normalize(offset, mcpTemp[mcpMinIndex], mcpTemp[mcpMaxIndex])
mcp_time_Norm = normalize(time[mcpMaxIndex:299], time[mcpMaxIndex], time[299])
mcp_Temptau = unnormalize(mcp_offset_Norm + 1/np.e, mcpTemp[mcpMinIndex], mcpTemp[mcpMaxIndex])
...
aht_ClosestIndex = find_closest_index(ahtTemp, ahtMaxIndex, 299, aht_Temptau)
aht_Tau = normalize(time[aht_ClosestIndex], time[ahtMaxIndex], time[299])

mcp_ClosestIndex = find_closest_index(mcpTemp, mcpMaxIndex, 299, mcp_Temptau)
mcp_Tau = normalize(time[mcp_ClosestIndex], time[mcpMaxIndex], time[299])
